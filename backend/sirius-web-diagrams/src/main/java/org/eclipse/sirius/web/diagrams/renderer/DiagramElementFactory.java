...
import org.eclipse.sirius.web.diagrams.components.LabelPlacementKind;
...
public class DiagramElementFactory implements IElementFactory {
    ...
    private Edge instantiateEdge(EdgeElementProps props, List<Object> children) {
        // @formatter:off
        Label beginLabel = this.getLabel(children, LabelPlacementKind.EDGE_BEGIN);
        Label centerLabel = this.getLabel(children, LabelPlacementKind.EDGE_CENTER);
        Label endLabel = this.getLabel(children, LabelPlacementKind.EDGE_END);
    ...
    private Label getLabel(List<Object> children, LabelPlacementKind labelPlacementKind) {
        // @formatter:off
        return children.stream()
                .filter(Label.class::isInstance)
                .map(Label.class::cast)
                .filter(label -> label.getType().equals(labelPlacementKind.getValue()))
                .findFirst()
                .orElse(null);
        // @formatter:on
